...
- Cache behavior ignores MTG Wiki HTTP cache headers and uses a 30-minute local scrape cache.
- Pass `--ignore-cache` after the pnpm script separator to force fresh wiki fetches, e.g. `pnpm generate:counter -- --ignore-cache`.
...
- Wiki page fetches for this generator reuse `/scripts/wiki-html-fetcher.ts` and `/scripts/mtg-wiki-cache` with the same 30-minute local scrape cache semantics as counter generation.

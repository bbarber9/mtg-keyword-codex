...
- Output format: `Record<string, { intro: string; description: string; reminderText: string; sourceUrl: string }>` where keys are display names from the root page list.
...
- Output format: `Record<string, { intro: string; description: string; reminderText: string; sourceUrl: string }>` where keys are display names from the root page rules list.
- Wiki page fetches for this generator reuse `/scripts/wiki-html-fetcher.ts` and `/scripts/mtg-wiki-cache` with the same browser-style cache semantics as counter generation.
